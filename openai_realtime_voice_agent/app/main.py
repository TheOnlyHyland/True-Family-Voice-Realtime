...
def parse_rapid_pilot_follow_up_seconds(value: Any) -> int:
    """Require the only supported 0.22.9 microphone mode."""
    if type(value) is int:
        ...
        raise ValueError(
            "follow_up_listen_seconds must be 0 exactly for the 0.22.9 rapid pilot; "
            "legacy automatic follow-up is disabled"
        )
    if seconds != 0:
        raise ValueError(
            "follow_up_listen_seconds must be 0 for the 0.22.9 rapid pilot; "
            "legacy automatic follow-up is disabled"
        )
    return 0
...
def validate_rapid_pilot_prerequisites(
    ...
    if turn_detection_type != "semantic_vad" or not backend_owned_response_creation:
        raise ValueError(
            "The 0.22.9 rapid pilot requires managed semantic_vad response creation"
        )
    if max_context_messages <= 0:
        raise ValueError(
            "The 0.22.9 rapid pilot requires max_context_messages greater than 0"
        )
...
def validate_selective_follow_up_media_scope(
    ...
    if request_follow_up_supported and not nearby_media_players:
        raise ValueError(
            ...
            "media_player.living_room_tv_audio for the 0.22.9 rapid pilot"
        )
...
class Application:
    ...
    async def initialize(self) -> None:
        ...
        # Version 0.22.9 is the serial explicit-follow-up pilot. Automatic mode
