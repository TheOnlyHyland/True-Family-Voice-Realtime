...
> released backend 0.22.8. A source checkout is not deployable. Rollback remains
...
4. Open the add-on's **Configuration** tab to set it up. For 0.22.9, update exact
...
| `max_context_messages` | `12` | complete user-led turns retained; version 0.22.9 requires a value above `0` |
...
Version 0.22.9 holds managed output until the exact terminal response structure
...
Leave them unset: version 0.22.9 requires managed `semantic_vad` and rejects
...
**Backend enrollment is not available in 0.22.9.** The add-on exposes no model,
