...
4. Open the add-on's **Configuration** tab to set it up. For 0.22.5, update exact
   firmware 0.20.1 first and install only the protected published backend image,
...
| `max_context_messages` | `12` | complete user-led turns retained; version 0.22.5 requires a value above `0` |
...
Version 0.22.5 holds managed output until the exact terminal response structure
...
Leave them unset: version 0.22.5 requires managed `semantic_vad` and rejects
...
**Backend enrollment is not available in 0.22.5.** The add-on exposes no model,
