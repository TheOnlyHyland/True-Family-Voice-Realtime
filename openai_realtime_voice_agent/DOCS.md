...
4. Open the add-on's **Configuration** tab to set it up. For 0.22.2, update exact
...
| `max_context_messages` | `12` | complete user-led turns retained; version 0.22.2 requires a value above `0` |
...
Leave them unset: version 0.22.2 requires managed `semantic_vad` and rejects
...
**Backend enrollment is not available in 0.22.2.** The add-on exposes no model,
