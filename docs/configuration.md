...
> Version 0.22.4 requires the default `semantic_vad`; selecting `server_vad`
> blocks startup.

> **Version 0.22.4 is bound to exact firmware 0.20.0.** Update and verify that
...
| Option | Default | Purpose / when to change |
|---|---|---|
| `follow_up_listen_seconds` | `0` | Must remain `0` in version 0.22.4. Ordinary replies close the mic. The model may request one useful no-wake question at a time and, after each genuine answer, request another without a round-count limit inside the existing 120-second physical wake. A saved nonzero value fails startup instead of enabling legacy automatic mode. |
...
> **Accepted inherited privileges:** 0.22.4 still uses host networking, the Home
...
| `max_context_messages` | `12` | Complete user-led turns retained across the hourly OpenAI reconnect; tool calls and results stay together. Version 0.22.4 requires a value above `0` so a requested conversational turn survives a fresh wake. |
...
| `turn_detection_type` | *(unset)* | Leave unset. The 0.22.4 rapid pilot requires managed `semantic_vad`; selecting legacy `server_vad` blocks startup. |
| `vad_threshold` | *(unset)* | Legacy `server_vad` saved-config field. Leave unset; using `server_vad` blocks 0.22.4 startup. |
| `vad_prefix_padding_ms` | *(unset)* | Legacy `server_vad` saved-config field. Leave unset; using `server_vad` blocks 0.22.4 startup. |
| `vad_silence_duration_ms` | *(unset)* | Legacy `server_vad` saved-config field. Leave unset; using `server_vad` blocks 0.22.4 startup. |
