...
> Update and verify firmware first, then install backend 0.22.2 only from its
...
### 1.5 Use only the protected 0.22.2 release artifact

Save the configuration, but do **not** install or start backend 0.22.2 from a
source checkout. Its binding is finalized to firmware 0.20.0, which must be
updated and verified first. Install backend 0.22.2 only after the protected image
...
published backend 0.22.2 image. If that release is not yet available, keep using
...
4. Ordinary replies close the mic. In 0.22.2, the assistant may request one
