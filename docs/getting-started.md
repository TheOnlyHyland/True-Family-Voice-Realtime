...
> Update and verify firmware first, then install backend 0.22.6 only from its
> protected published image. Until that image and GitHub release exist, keep
> using released backend 0.22.5. A source checkout is not
...
### 1.5 Use only the protected 0.22.6 release artifact

Save the configuration, but do **not** install or start backend 0.22.6 from a
source checkout. Its binding is finalized to firmware 0.20.2, which must be
updated and verified first. Install backend 0.22.6 only after the protected image
and GitHub release exist; otherwise remain on released backend 0.22.5.
...
published backend 0.22.6 image. If that release is not yet available, keep using
released backend 0.22.5 rather than a source checkout. Click **Start** on the
...
4. Ordinary replies close the mic. In 0.22.6, the assistant may request one
