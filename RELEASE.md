...
The exact Voice PE firmware binding for backend 0.22.1 is finalized:
...
> verify exact firmware 0.20.0, then update to backend 0.22.1 only after its
> protected images and GitHub release exist. Starting backend 0.22.1 before the
...
auditability but cannot authorize a 0.22.1 image or public GitHub release. Every
0.22.1 publication requires `pilot_firmware_source_only=false`, a finalized public
...
For the response to one freshly confirmed follow-up answer, backend 0.22.1 holds
...
Backend 0.22.1 binds each assistant PCM source frame, Pipecat chunker operation,
...
Version 0.22.1 deliberately inherits the pilot's `host_network: true`,
...
2. Manually dispatch **Build and Publish Home Assistant Addon** from that exact
   candidate commit with `publish=true`, `release_tag=v0.22.1`, and
...
7. Tag the reviewed candidate commit `v0.22.1`; only then publish the GitHub release.
   The release event is verification-only and must not build or push an image.

The publication workflow refuses to overwrite either an existing `0.22.1` tag
